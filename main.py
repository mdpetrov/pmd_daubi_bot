...
from pmd_daubi_bot.lfp_helpers import (
    ...
    LFP_ACTION_JOIN,
    LFP_ACTION_LEAVE,
    LFP_CLOSE_REASON_MANUAL,
    lfp_add_attendee,
    lfp_build_keyboard,
    lfp_clear_old_sessions,
    lfp_close_reason,
    lfp_close_session,
    lfp_create_session,
    lfp_get_active_session,
    lfp_parse_request,
    lfp_remove_attendee,
    lfp_render_text,
    lfp_render_existing_session_notice,
    lfp_store_session,
...
def get_message_lfp(message):
    ...
    active_session = lfp_get_active_session(local_params)
    if active_session:
        BO.send_message(
            message.chat.id,
            text=lfp_render_existing_session_notice(active_session),
            params=local_params,
            parse_mode='HTML',
        )
        PO.save_params(message.chat.id, local_params)
    ...
    parsed, error = lfp_parse_request(args_text, time.time(), config.param_value)
    if error:
        BO.send_message(
            message.chat.id,
            text=f'{error}\n\n{config.param_value["lfp_usage_hint"]}',
            params=local_params,
        )
        PO.save_params(message.chat.id, local_params)
        return

    session = lfp_create_session(message.chat.id, 0, message.from_user, parsed, time.time())
    msg = BO.send_message(
        message.chat.id,
    ...
    try:
        ...
        LO.write_log(message.chat.id, f'LFP pin message failed: {e}')

    lfp_clear_old_sessions(local_params)
    lfp_store_session(local_params, session)
    PO.save_params(message.chat.id, local_params)
    close_reason = lfp_close_reason(session, time.time())
    if close_reason:
        lfp_close_session(bot, BO, PO, LO, message.chat.id, local_params, session, close_reason)
...
def handle_lfp_callback(call):
    ...
    session = lfp_get_active_session(local_params)
    if not session or session.get('session_id') != session_id:
        bot.answer_callback_query(call.id, 'Сбор уже закрыт или не найден')
        return

    close_reason = lfp_close_reason(session, time.time())
    if close_reason:
        lfp_close_session(bot, BO, PO, LO, chat_id, local_params, session, close_reason)
        bot.answer_callback_query(call.id, 'Сбор закрыт')
        return

    if action == LFP_ACTION_JOIN:
        _, answer_text = lfp_add_attendee(session, call.from_user)
        bot.answer_callback_query(call.id, answer_text)
    elif action == LFP_ACTION_LEAVE:
        _, answer_text = lfp_remove_attendee(session, call.from_user)
        bot.answer_callback_query(call.id, answer_text)
    ...
    try:
        ...
        LO.write_log(chat_id, f'LFP edit poll message failed: {e}')
        
def lfp_autoclose_loop():
    while True:
        try:
            data_dir = config.path['data_dir']
            if os.path.isdir(data_dir):
                for file_name in os.listdir(data_dir):
                    if not file_name.endswith('.param'):
                        continue
                    chat_id = file_name[:-6]
                    local_params = PO.load_params(chat_id)
                    lfp_prune_and_autoclose(bot, BO, PO, LO, chat_id, local_params, config.param_value)
        except Exception as e:
            logging.warning('LFP autoclose loop failed: %s', e)
        time.sleep(config.param_value['lfp_check_interval_seconds'])


def start_lfp_autoclose_loop():
    worker = threading.Thread(target=lfp_autoclose_loop, daemon=True)
    worker.start()


if __name__ == '__main__':
    LO.write_log(0, 'Start the bot')
    start_lfp_autoclose_loop()
